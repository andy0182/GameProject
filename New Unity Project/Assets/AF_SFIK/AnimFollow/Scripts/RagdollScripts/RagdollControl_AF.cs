...
namespace AnimFollow
{
	...
	//	 This kind of a state machine that takes the character through the states: colliding, falling, matching the masters pose and getting back up
		...
		void Awake () // Initialize
		{
			...
			{
				noIceDynFriction[m] = IceOnGetup[m].collider.material.dynamicFriction;
				noIceStatFriction[m] = IceOnGetup[m].collider.material.staticFriction;
			}
		...
		public IEnumerator Falling()
		{
			...
			{
				IceOnGetup[m].collider.material.dynamicFriction = noIceDynFriction[m];
				IceOnGetup[m].collider.material.staticFriction = noIceStatFriction[m];
			}
			animFollow.angularDrag = angularDrag;
			animFollow.drag = drag;
			while(ragdollRootBone.rigidbody.velocity.magnitude > settledSpeed)
			{
			...
			yield return StartCoroutine(Orientate ());
			isOverride = false;
		}
		IEnumerator Orientate()
		{
			do 
			{
				...
				yield return null;
			} while (!(!isInTransitionToGetup && wasInTransitionToGetup));
			...
			{
				IceOnGetup[m].collider.material.dynamicFriction = 0f;
				IceOnGetup[m].collider.material.staticFriction = 0f;
			}
			animFollow.angularDrag = getupAngularDrag;
			animFollow.drag = getupDrag;
			while (animFollow.maxTorque < wakeUpStrength) // Ease the ragdoll to the master pose. WakeUpStrength limit should be set so that the radoll just has reached the master pose
			{
			...
			do
			{
				...
				yield return null;
			} while(!(limbErrorMagnitude < maxErrorWhenMatching));
		}
		//bool isStart=true;
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//		Debug.DrawRay(ragdollRootBone.position, ragdollRootBone.rotation * rootboneToForward * Vector3.forward); // Open this and check that the ray is pointing as the nose of the charater
			...
			{	
				...
				else // Falling
				{
					...
					if (ragdollRootBone.rigidbody.velocity.magnitude < settledSpeed) // && contactTime + noContactTime > .4f)
					{
						gettingUp = true;
						orientate = true;
...
//						}
		...
		bool WeHaveAllTheStuff()
		{
			...
			else
			{
				...
				else if (!ragdollRootBone.rigidbody || !(ragdollRootBone.root == this.transform.root))
				{
					ragdollRootBone = GetComponentInChildren<Rigidbody>().transform;
